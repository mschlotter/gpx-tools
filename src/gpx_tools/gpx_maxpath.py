...
def find_longest_path(segment, edges):
    ...
    # dpX_lengths[j,X] stores the max length of an X-edge path ending at point P[j]
        ...
        for j in range(edge+1, N):   # P[j] is endpoint of the edge
            for i in range(edge, j): # P[i] is startpoint of the edge
                if edge == 0:  # path length of first edge 
                    path_length = haversine_distance(segment.points[i], segment.points[j])
                else:          # path length with multiple edges
                    if dp_parents[i, edge-1] == -1:  # make sure, start is valid
                        continue
                    path_length = dp_lengths[i,edge-1] + \
                        haversine_distance(segment.points[i], segment.points[j])
...
def generate_filename(data, input_filename, max_length):
    ...
    # filename
    filename = date + "_" + os.path.splitext(input_filename)[0] + "_" + \
        competition_id + "_" +  str(int(max_length)) + ".gpx"
...
def main(args):
    ...
    dist = args.dist if args.dist is not None else config.get('dist', 100) if config else 100
    ...
    for input_path in input_paths:
        ...
        if data.routes:
            for rtp in data.routes[0].points:
                data.waypoints.append(
                    gpxpy.gpx.GPXWaypoint(rtp.latitude, 
                                        rtp.longitude, 
                                        name=rtp.name, 
                                        comment=rtp.comment, 
        ...
        for ptp in path_points:
            route.points.append(
                gpxpy.gpx.GPXWaypoint(ptp.latitude, 
                                    ptp.longitude,
                                    ptp.elevation,
                                    ptp.time))
        ...
        relative_path_dir = os.path.relpath(os.path.dirname(input_path), input_directory)
